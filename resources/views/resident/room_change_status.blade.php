...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function fetchRoomChangeRequests(residentId) {
        fetch(`{{ url('/api/resident') }}/${residentId}/room-change-requests`)
            .then(response => {
                if (!response.ok) {
    ...
    window.confirmRequest = function (requestId) {
        fetch(`{{ url('/api/room-change/confirm-by-resident') }}/${requestId}`, {
            method: 'POST',
            headers: {
